...
async function checkScores() {
  ...
  // 比賽結束判定：只有「狀態明確變成比賽結束(FT)」或「整場從列表消失」才算疑似結束，
  // 並需二次輪詢確認才通知。雨停/中斷(比賽暫停)、延賽、取消都「不算結束」，
  // 避免把中途中斷或延賽的場次凍結比分誤報成最終結果。
  for (const [gameId, oldSnap] of Object.entries(oldState)) {
    ...
    if (oldSnap.pendingEnd) {
      if (looksEnded) {
        ...
        logger.log(`場次 ${gameId} 結束誤判，比賽尚未結束（狀態=${current?.status ?? '消失'}），清除暫定結束標記`);
        if (newState[gameId]) newState[gameId].pendingEnd = false;
      }
      continue;
    }
